...
# args.width = 252
# args.size = [252, 312]
# args.init_image = "/root/me.jpeg"
# args.prompts = "blue → white"
...
i = 0
try:
    with tqdm(max_iterations) as pbar:
        while True:
            train(i)
            if i == max_iterations:
                break
...
p = Popen(['ffmpeg', '-y', '-f', 'image2pipe', '-vcodec', 'png', '-r', str(fps), '-i', '-', '-vcodec', 'libx264', '-r',
           str(fps), '-pix_fmt', 'yuv420p', '-crf', '17', '-preset', 'veryslow', video_filename], stdin=PIPE)
